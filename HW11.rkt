...
(define run-parsed-code
  (lambda (parsed-no-code env)
    (cond
      ...
      ((eq? (car parsed-no-code) 'let*-exp)
       (let* ((new-scope (construct-scope (cdr (car parsed-no-code)) env))
              (new-env (extend-env new-scope env))
              (body (caddr parsed-no-code)))
         (run-parsed-code body new-env)))
...
(define sample-let-no-code '(local-vars ((a 2) (b 3)) (local-vars ((a (do-mathy-stuff * a 4)) (r 5)) (do-mathy-stuff + a b))))
(define parsed-no-code (no-parser sample-no-code))
(define sample-let-no-code-two '(local-vars ((a 2) (b a)) b))
(define parsed-no-code-let (no-parser sample-let-no-code-two))
;parsed-no-code
(run-parsed-code parsed-no-code env)
(run-parsed-code parsed-no-code-let env)